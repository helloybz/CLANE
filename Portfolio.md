...
일반적으로 주어진 데이터에 내재되어있는 특징적인 패턴을 찾아내기 위해서는 머신러닝 메소드 적용합니다.
그런데 네트워크 데이터는 그 구조적 특성상 머신 러닝 메소드를 직접 적용하기가 어렵습니다.
네트워크 데이터의 자료구조인 그래프는 정점들(vertices, nodes)의 집합과 정점들 사이를 연결하는 간선들(edges, links)의 집합으로 구성되어 있으며, **관계**라는 복잡한 정보를 나타내기에 유용합니다.
머신러닝 메소드는 일반적으로 수치 데이터를 그 입력으로 하는 반면, '정점들 사이의 **관계**'는 수치 형태가 아닙니다.
머신러닝 메소드를 적용할 수 있도록 정점들 사이의 **관계**를 수치화하는 것이 **네트워크 임베딩**기법입니다.

[**임베딩(Embedding)**](https://en.wikipedia.org/wiki/Embedding)이란 수치 형태로 표현되지 않은 데이터를 임의의 벡터 공간에 할당하여 수치화하는 과정, 또는 할당된 값 자체를 의미합니다.
예를 들어, 10개의 단어를 1차원의 벡터 공간에 할당하면 수직선 위에 10개의 점들이 표시될 것입니다.
각각의 점들은 단어 하나에 해당하며, 수직선상의 점의 위치는 해당 단어의 임베딩이 됩니다.
수직선 상에 아무렇게나 점 10개를 찍을 것이 아니라, 각 단어의 특성을 반영하면 좋겠습니다.
단어의 특성이 임베딩에 잘 반영되었다면, 의미적으로 비슷한 단어는 수직선상에서의 거리가 가까울 것이고, 그렇지 않은 단어는 그 거리가 멀 것입니다.

네트워크 임베딩도 이와 유사합니다.
단어 대신에 네트워크를 이루는 정점들을 임의의 벡터 공간에 할당하되, 비슷한 정점들은 가깝게, 그렇다 않은 정점들은 멀리 할당합니다.
이후 모든 정점이 임베딩을 갖게 되면 이 임베딩을 기반으로 머신 러닝 모델을 훈련하여 분류, 군집화, 예측 등 다양한 분석을 수행할 수 있습니다.

![네트워크 데이터(좌)와 네트워크 임베딩(우)의 예시](https://github.com/helloybz/CLANE/raw/develop/resources/Deepwalk_Karate.png)_그림 1. 네트워크 데이터(좌)와 네트워크 임베딩(우)의 예시_
...
Ground Truth는 임베딩 과정에는 사용되지 않고, 임베딩 과정을 마친 후 임베딩의 성능을 평가하기 위해 사용됩니다.
임베딩 결과를 보면 2차원 공간에 같은 색깔의 정점끼리 모여 있어 네트워크의 구조적 정보가 임베딩에 잘 반영되었음을 알 수 있습니다.
